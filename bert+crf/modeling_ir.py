...
class IntentionLabelTagger(Model):
    ...
    def forward(self, dialogue, speaker, intentions = None, actions = None, **kwargs,):
        ...
        # encoded_utterance = list(encoded_utterance.hidden_states)
        # last_hidden_states = encoded_utterance[-1]
        # first_hidden_states = encoded_utterance[0]
        # first_last_all_status = torch.add(last_hidden_states, first_hidden_states)
        # first_last_avg_status = torch.mean(first_last_all_status,dim=1)
        # encoded_utterance = first_last_avg_status
        # encoded_utterance = encoded_utterance.reshape(batch_size, utter_len, -1)
        ...
        # 修改，transformer需要mask
        encoded_dialogue = self.dialogue_encoder(inputs=encoded_utterance, mask=torch.ones(encoded_utterance.shape[:-1]).to(encoded_utterance.device)!=0)
        ...
        if actions != None:
            # class_probabilities = encoded_dialogue_act
        ...
        if intentions != None:
            # class_probabilities = encoded_dialogue_int
