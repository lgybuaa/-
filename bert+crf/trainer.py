...
def build_model(vocab: Vocabulary,
    ...
    lstmencoder = LstmSeq2SeqEncoder(input_size=pretrained_hidden_size,
                                     ...
                                     num_layers=1,
                                     bidirectional=True)
    transformerencoder = PytorchTransformer(input_dim=pretrained_hidden_size,
                                            num_layers=2,
                                            num_attention_heads=4,
                                            feedforward_hidden_dim=64,
                                            positional_encoding='embedding',)  # sinusoidal,embedding
    # 修改，使用transformer encoder架构处理对话序列
...
def build_trainer(model: Model,
    ...
    # no_bigger = ["crf_act", "crf_int", "act_decoder", "intent_decoder"]
